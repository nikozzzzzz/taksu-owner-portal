...
echo "[$(date)] Refreshing Beds24 token (heartbeat)..." >> logs/cron_sync.log
...
if [ "$REFRESH_STATUS" -eq 200 ]; then
  echo "[$(date)] Token refresh succeeded." >> logs/cron_sync.log
elif [ "$REFRESH_STATUS" -eq 404 ]; then
  echo "[$(date)] WARNING: No Beds24 credentials stored — skipping sync. Please reconnect via Admin > Integrations." >> logs/cron_sync.log
  exit 0
else
  echo "[$(date)] WARNING: Token refresh returned HTTP $REFRESH_STATUS: $REFRESH_BODY — attempting sync anyway." >> logs/cron_sync.log
fi

# ── Step 2: Trigger full booking/price sync ───────────────────────────────────
echo "[$(date)] Starting automated Beds24 booking + price sync..." >> logs/cron_sync.log
RESPONSE=$(curl -s -w "\nHTTP_STATUS:%{http_code}" -X POST \
  -H "$AUTH_HEADER" \
  -H "Content-Type: application/json" \
